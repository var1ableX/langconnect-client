...
export class SupabaseAuthProvider implements AuthProvider {
  ...
  async signIn(credentials: AuthCredentials) {
    try {
      console.log("[SupabaseAuth] Calling signInWithPassword...");
      const { data, error } = await this.supabase.auth.signInWithPassword({
        email: credentials.email,
        password: credentials.password,
      });

      console.log("[SupabaseAuth] signInWithPassword response:", {
        hasData: !!data,
      ...
      // Explicitly refresh session to ensure it's persisted in cookies
      if (data.session) {
        console.log("[SupabaseAuth] Session obtained, refreshing...");
        const { data: refreshedSession, error: refreshError } = 
          await this.supabase.auth.getSession();
        
        console.log("[SupabaseAuth] getSession response:", {
          hasSession: !!refreshedSession?.session,
          hasError: !!refreshError,
        });

        if (refreshError) {
          console.error("[SupabaseAuth] Refresh error:", refreshError);
          throw refreshError;
        }

        const result = {
          user: this.formatUser(data?.user),
          session: this.formatSession(refreshedSession?.session),
          error: null,
        };
        
        console.log("[SupabaseAuth] Returning refreshed session");
        return result;
      }

      console.log("[SupabaseAuth] Returning original session");
      return {
        user: this.formatUser(data?.user),
        session: this.formatSession(data?.session),
        error: null,
